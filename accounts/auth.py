...
class Authentication:
    def signin(self, email=None, password=None) -> User | None:
        ...
        exception_auth = AuthenticationFailed('Email ou senha incorreto')
    ...
    def signup(self, name, email, password, type_account='owner', company_id=False):
        if not name or name=='':
            raise APIException('Nome não deve ser Null')
        
        if not email or email=='':
            raise APIException('Email não deve ser Null')
        
        if not password or password=='':
            raise APIException('Senha não deve ser Null')
        
        if type_account =='employee' and not company_id:
            raise APIException('Id da empresa não deve ser Null')

        user = User
        if user.objects.filter(email=email).exists():
            raise APIException('Email já existe')
